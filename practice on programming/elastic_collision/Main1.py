...
def spawn_ball(mouse_x, mouse_y, x, y):
    ...
    if click and not clicked:
        ...
        clicked = True
        x, y = mouse_x, mouse_y
        init_ball = Ball2.ball(mouse_x, mouse_y, 0, 0, time.time(), screen)
...
def main():
    ...
    balls = []
    init_ball = Ball2.ball(0, 0, 0, 0, time.time(),screen)
    #=============================Процесс
    while flag:
        screen.fill((0, 0, 0))
        myfont = pygame.font.SysFont('Arial', 30)
        textsurface2 = myfont.render(f'Вместимость: {1}', False, (255, 255, 255))
